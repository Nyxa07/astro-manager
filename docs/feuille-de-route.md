...
- **Socle** — trois frontières, contrat IPC dérivé d'`IPC`, preload générique, verrou de navigation, `database.ts` (migrations sous `PRAGMA user_version`, `application_id` `ASTM`), deux suites de specs. `protocols.ts` déclare les privilèges des schémas puis branche les handlers ; `serve.ts` rend un fichier en `Response` de flux, sans le charger ; `ipc.ts` est scindé en `createModules(deps)` et `registerIpcHandlers(modules)`.
...
- **`picture`** — `picture:scan`, `picture:list` et l'arête `workspace://`. Migration v2 (table `picture`), `kind` (sorte par extension, `MIME_BY_KIND`), `walk` (générateur asynchrone d'événements `added | changed | removed`), `store` (`list`, `known`, `apply` en une transaction, `findById`, `findByPath`). Pour les vignettes : `url.ts` (parseur pur des deux hôtes), `thumb.ts` (`ensure`, `invalidate`, `purge`), `fits.ts` (lecteur maison) et `raster.ts`. Côté renderer : injectable `Picture` (`all` en `resource` sur l'espace courant, `scan()`, `scanning`, `selected` en `linkedSignal`, `select()`), sans écran.
...
3. **`workspace://` et vignettes** — _livrée le 22 septembre 2026_. Deux hôtes, `file/<chemin relatif>` et `thumb/<id>` ; parseur pur, réponse en flux par `serve`, privilèges `standard` + `secure` + `stream`, CSP `img-src 'self' data: workspace:`. Vignettes `.astro-manager/thumbs/<id>.jpg` bornées à 360 × 240, générées sur défaut de cache et écrites en `.part` puis renommées : FITS par un lecteur maison (en-tête, lignes échantillonnées et colonnes moyennées, étirement par percentiles, superpixel si `BAYERPAT`), JPEG/PNG par `nativeImage`, rien pour RAW/TIFF — 404, la tuile de remplacement revient au renderer. Échec mémorisé pour la session, orphelines purgées au balayage. Seconde arête du module ; `architecture.md` suit.
...
- **Injecter ce que la spec doit remplacer, importer ce qui est pur et présent.** Un effet, une absence du process de test, un axe de variation gelé par le process : `fs`, `dialog`, `nativeImage`, `path` (pour atteindre `path.win32`). Le reste s'importe — `store`, `kind`, `url` sont des fonctions pures, les injecter n'ajouterait qu'une indirection.
- **Les modules sont construits une fois.** `createModules(deps)` rend un enregistrement nommé (`ReturnType` en guise de type) que `registerIpcHandlers` et `registerProtocols` consomment tous les deux. Deux constructions donneraient deux caches de vignettes, dont l'un mémoriserait des échecs que l'autre ignore.
- **`workspace://file/` prend un chemin, pas un id, et le catalogue reste l'autorité.** L'URL d'un original se fabrique à partir de ce que le renderer a déjà — `PictureInfo.path` —, mais rien n'est servi qui n'ait sa ligne : `library.db` est sous la racine et reste inatteignable.
- **Une `Response` ne se réutilise pas.** Son corps est un flux à usage unique et ses en-têtes sont mutables : les réponses vides sont fabriquées par une fonction (`empty(status)`), jamais partagées en constante.
- **Les vignettes ignorent `BZERO` et `BSCALE`.** L'étirement par percentiles est invariant par transformation affine : les appliquer changerait les nombres sans changer l'image. Un échec de rendu est mémorisé **pour la session**, en mémoire, jamais sur le disque.
...
- Une vignette ne s'invalide que si son fichier change, jamais si l'algorithme de rendu change : l'ancienne reste servie. Un numéro de version dans le chemin du cache (`thumbs/v2/`) ?
- `thumb.invalidate` et `thumb.purge` sont attendus après le `COMMIT` de `scan` : un défaut d'entretien du cache fait échouer un balayage par ailleurs réussi. Les détacher, ou les laisser porter l'échec ?
- `new URL` normalise `..` avant tout contrôle : la traversée est arrêtée par le catalogue, pas par le parseur. Suffisant tant que `file/` passe par `findByPath` ; à revoir si un hôte servait un jour un chemin sans ligne.
