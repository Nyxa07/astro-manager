...
3. **`workspace://` et vignettes** — _livrée le 22 septembre 2026_. Deux hôtes, `file/<chemin relatif>` et `thumb/<id>` ; parseur pur, réponse en flux par `serve`, privilèges `standard` + `secure` + `stream`, CSP `img-src 'self' data: workspace:`. Vignettes `.astro-manager/thumbs/<id>.jpg` bornées à 360 × 240, générées sur défaut de cache et écrites en `.part` puis renommées : FITS par un lecteur maison (en-tête, lectures échantillonnées, étirement par percentiles, superpixel si `BAYERPAT`), JPEG/PNG par `nativeImage`, rien pour RAW/TIFF — 404, la tuile de remplacement revient au renderer. Échec mémorisé pour la session, orphelines purgées au balayage. Seconde arête du module ; `architecture.md` suit.
