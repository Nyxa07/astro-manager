...
4. `src/main/ipc.ts` — ajouter le module à `createModules`, puis l'étaler dans la composition de `registerIpcHandlers`.
...
Un validateur **partagé** entre plusieurs canaux se type sur l'union **calculée** des canaux concernés — `NoArgChannel` et `RootChannel` dans `workspace`, dérivés du contrat par `Parameters<IpcContract[C]> extends []` et `extends [string]` — jamais sur l'union de tous les canaux du module. Comparer les **tuples de paramètres**, pas les formes de fonction : `IpcContract[C] extends (root: string) => unknown` est vrai aussi pour les canaux sans argument, une fonction à zéro paramètre étant assignable à une fonction qui en prend un, et l'union avalerait tout le module sans que rien ne rougisse. Typé sur l'union entière, `Validator<A | B | C>` est accepté pour chaque canal pris séparément, `reopen` compris : TypeScript compare deux instanciations d'un même alias par la variance mesurée de `C`, la mesure conclut « contravariant » sans revérifier structurellement, et le corps du validateur n'est plus vérifié que contre l'union des tuples. Le `satisfies` de composition ne rattrape rien : les constantes étaient déjà déclarées du bon type.
...
`main/modules/` ne contient que des **arêtes** : ce qui reçoit un appel — IPC ou protocole —, valide, délègue, répond. Tout ce dont une arête dépend — `dialog`, fichiers, base — lui est **injecté par sa fabrique**, typé au plus étroit (`Pick<typeof dialog, 'showOpenDialog'>` plutôt que `Dialog`). C'est ce qui rend l'arête testable sans Electron ni disque, et c'est la pression de test qui fait tenir la règle : une arête qui ferait une E/S directe ne pourrait plus se tester en isolation.

**Le critère d'injection : on injecte ce que la spec doit remplacer, on importe ce qui est pur et présent.** Un effet (`fs`, `dialog`), quelque chose d'absent du process de test (`nativeImage` n'existe pas hors d'Electron), ou un axe de variation que le process gèle (`path`, injecté pour que `walk.spec` vérifie le cas Windows par `path.win32`) : ces trois-là s'injectent. Le reste s'importe — `store`, `kind`, `url` sont des fonctions pures du même module, les injecter n'ajouterait qu'une indirection sans rien rendre testable. C'est pourquoi `picture` reçoit `fs` mais importe `path` dans son `index.ts` : les deux traitements ne sont pas une incohérence, ils répondent différemment à la même question.
...
- Un collaborateur remonte à la racine de `main/` quand un second module en a besoin **et** qu'il n'a plus de propriétaire naturel — c'est le cas de `database.ts` et de `session.ts`. La racine ne grossit que par promotion. Une dépendance de domaine entre modules s'importe depuis l'`index.ts` de l'autre module, jamais depuis ses fichiers privés.
- `ipc.ts` est la **racine de composition**, en deux temps. `createModules(deps)` reçoit les dépendances réelles, construites par `index.ts` à `app.whenReady()` — `dialog`, `userData`, `nativeImage`, la session —, et rend un enregistrement **nommé** dont le type est `ReturnType<typeof createModules>` : pas un tableau, qui effacerait le type de chaque élément et rendrait l'exhaustivité indémontrable. `registerIpcHandlers(modules)` compose et enregistre ensuite. La séparation existe parce qu'un module peut porter **plus que des canaux** : `registerProtocols(modules, …)` consomme le même enregistrement, et `picture` n'est donc construit qu'une fois — deux `createPictureModule` donneraient deux caches de vignettes. Les specs passent de fausses dépendances à `createModules`. Les modules sans dépendance (`version`) restent des constantes. Un collaborateur que deux modules doivent **partager** est construit là-haut et injecté aux deux fabriques ; un module ne construit jamais ce qu'un autre devra voir.

### La session

`src/main/session.ts` est le **contexte de la bibliothèque ouverte** : `current()` rend `OpenWorkspace = { info, db }` — l'identité de l'espace et la connexion à son catalogue — ou `null`. Construite une fois dans `index.ts`, passée à `registerIpcHandlers` puis à chaque fabrique qui la demande : deux arêtes ne partagent une instance que si quelqu'un au-dessus l'a construite.

- **Qui écrit se lit dans les deps, pas dans l'emplacement.** `workspace` reçoit `Pick<Session, 'open' | 'current'>` — le seul qui ouvre ; les autres, `Pick<Session, 'current'>`. Personne ne ferme : `OpenWorkspace['db']` est `Pick<DatabaseSync, 'prepare' | 'exec'>`.
- **Le rétrécissement est un fait de type, pas d'objet.** `current()` rend l'instance `DatabaseSync` elle-même, vue à travers le `Pick` — comme `dialog` n'a jamais été recopié. Recopier les méthodes dans un objet neuf les détache de leur receveur, et une méthode native lève `Illegal invocation` : `Pick` décrit une forme, il n'attache rien, et `@types/node` ne déclare pas `this`. `session.spec.ts` appelle `db.prepare` à travers `current()` pour cette raison.
- **La connexion n'est pas un secret, c'est le contrat.** Exposer des méthodes typées par module (`pictures()`, `catalog()`) obligerait la session à connaître ses consommateurs — le sens interdit du graphe. Ce qui garde la base propre tient en quatre règles : chaque module **possède les écritures de ses tables** — les lectures croisées, un `JOIN`, sont permises ; le schéma est un contrat partagé, `MIGRATIONS` dans `database.ts` en est la source unique et linéaire, comme `shared/ipc.ts` pour les canaux ; **le SQL vit dans un collaborateur** (`<nom>/store.ts`), jamais dans l'arête ; **personne ne garde `db`** — on relit `session.current()` à chaque appel, une connexion capturée dans la fabrique est morte à la première réouverture ; **une transaction ne traverse jamais un `await`** — une seule connexion synchrone, un appel IPC arrivé pendant l'attente tomberait dedans.
- **La table `workspace` est celle de la session**, pas d'un module : la carte d'identité de la base, écrite une fois à la création, du même ordre qu'`application_id` et `user_version`. Le jour où l'utilisateur renomme un espace, cette écriture-là ira dans le module `workspace`, par `db`, comme n'importe quelle autre.
- **`db` ne traverse jamais le pont.** Le handler `workspace:current` ne rend que `info` ; le type de fil `WorkspaceInfo | null` refuse `OpenWorkspace` à la compilation (`TS2322`), avant que structured clone ne s'y casse à l'exécution.

### Les protocoles

`src/main/protocols.ts` tient les deux schémas de l'application. Il y a **deux moments**, et les confondre est l'erreur la plus facile : `registerSchemesAsPrivileged` doit être appelé **avant** `app.whenReady()` — Chromium fige la table des schémas au démarrage —, `protocol.handle` **après**, parce qu'il a besoin de la session par défaut. D'où deux exports, `registerProtocolSchemes()` au niveau module d'`index.ts` et `registerProtocols(modules, deps)` dans `whenReady`, avant `createWindow()` : la fenêtre charge `app://local` dès sa création et ne doit rien trouver de manquant.

Les privilèges sont déclaratifs et chacun coûte quelque chose : `standard` donne au schéma une vraie grammaire d'URL (hôte, chemin) et une origine non opaque ; `secure` le fait traiter comme HTTPS ; `stream` autorise un corps en flux et les requêtes `Range`. `app` ajoute `supportFetchAPI` pour que le renderer puisse `fetch` ses propres ressources ; `workspace` ne l'a pas — rien ne le `fetch`, seules des balises `<img>` le lisent.

- **`serve` est le collaborateur commun**, à la racine de main : `serve(file, type)` ouvre le fichier, rend 404 sur `ENOENT`/`ENOTDIR` ou si ce n'est pas un fichier régulier, et sinon une `Response` dont le corps est `Readable.toWeb(handle.createReadStream())` — un FITS de 200 Mo ne passe jamais en mémoire. Toute réponse porte `Cache-Control: no-cache` : ce qu'il sert peut changer sous la même URL (`index.html` à chaque build, une vignette régénérée), et la politique est écrite plutôt que laissée à l'heuristique de Chromium. Le descripteur est fermé sur tout chemin d'échec ; le `try` qui attrape `ENOENT` n'entoure que l'`open`, jamais la suite.
- **Le type MIME est un argument, pas une déduction.** `serve` ne devine rien : `app` le tire de l'extension du fichier **servi** (repli SPA compris — `/pictures` rend `index.html`, donc `text/html`), `workspace://file/` de la sorte cataloguée (`MIME_BY_KIND`, un `Record<PictureKind, string>` pour que l'ajout d'une sorte soit une erreur de compilation).
- **Un module peut porter une arête de protocole** en plus de ses canaux : `picture` expose `protocol(request)` à côté de `handlers` et `validators`. Le parseur d'URL (`url.ts`) est **pur**, comme `resolveRendererFile` — ni Electron ni disque —, et rend une union discriminée (`{ ok: true; host: 'thumb'; id }`, `{ ok: true; host: 'file'; path }`, `{ ok: false; status: 400 }`) que l'arête dispatche par un `switch` exhaustif.
- **Le catalogue est l'autorité.** `thumb/<id>` et `file/<chemin>` passent tous deux par `store` ; ce qui n'a pas de ligne n'est pas servi, même si le fichier existe sous la racine. C'est ce qui rend `library.db` inatteignable, et c'est aussi ce qui arrête une traversée : `new URL` normalise `..` avant tout contrôle, donc `workspace://file/../../etc/passwd` devient `etc/passwd` — sans ligne au catalogue, 404.
- **La session se relit à chaque requête**, jamais capturée dans la fabrique : le protocole est enregistré une fois pour la vie de l'application, et l'espace ouvert change.
...
- `src/main/modules/<nom>.spec.ts` — handlers et validateurs, testés sur `<nom>Module` directement. Les collaborateurs d'un dossier ont chacun leur spec (`registry.spec.ts`) ; les collaborateurs promus aussi (`database.spec.ts`, `session.spec.ts`), sur de vrais dossiers temporaires et de vraies bases. Une fabrique reçoit une vraie session construite par la spec, comme `index.ts` le fait — et la spec la regarde de l'extérieur.
...
- **Le renderer n'envoie jamais un chemin de fichier.** Il demande l'ouverture d'un sélecteur ; c'est le process principal qui appelle `dialog.showOpenDialog`, et seul le chemin qui en sort est fiable. Les images lui parviennent par `workspace://`, résolu par main sous la racine courante et contre le catalogue — voir « Les protocoles ».
- La CSP du renderer autorise `workspace:` **sur `img-src` seulement** (`src/renderer/index.html`). `'self'` vaut `app://local` ; le schéma est une autre origine, et rien d'exécutable ne doit venir de l'espace de l'utilisateur. Le build de production n'inline pas le CSS critique (`inlineCritical: false` dans `angular.json`) : Angular chargerait ensuite la feuille complète par un `onload` inline, que `script-src 'self'` refuse — seules les règles du premier affichage s'appliqueraient. Le gain visé est réseau ; ici la feuille est lue sur le disque.
