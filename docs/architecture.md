...
- `ipc.ts` est la **racine de composition** : `registerIpcHandlers(deps)` reçoit les dépendances réelles, construites par `index.ts` à `app.whenReady()` — `dialog`, `userData`, la session —, fabrique les modules, compose, enregistre. Les specs lui passent des fausses. Les modules sans dépendance (`version`) restent des constantes. Un collaborateur que deux modules doivent **partager** est construit là-haut et injecté aux deux fabriques ; un module ne construit jamais ce qu'un autre devra voir.
