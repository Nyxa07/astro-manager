...
- La CSP du renderer autorise `workspace:` **sur `img-src` seulement** (`src/renderer/index.html`). `'self'` vaut `app://local` ; le schéma est une autre origine, et rien d'exécutable ne doit venir de l'espace de l'utilisateur.
