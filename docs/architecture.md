...
- **`serve` est le collaborateur commun**, à la racine de main : `serve(file, type)` ouvre le fichier, rend 404 sur `ENOENT`/`ENOTDIR` ou si ce n'est pas un fichier régulier, et sinon une `Response` dont le corps est `Readable.toWeb(handle.createReadStream())` — un FITS de 200 Mo ne passe jamais en mémoire. Le descripteur est fermé sur tout chemin d'échec ; le `try` qui attrape `ENOENT` n'entoure que l'`open`, jamais la suite.
